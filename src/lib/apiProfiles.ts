...
/** Removed built-in provider ID. Kept only to quarantine and discard legacy persisted/imported data. */
export const LEGACY_FAL_PROVIDER_ID = 'fal'
const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', LEGACY_FAL_PROVIDER_ID])
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  const targetProvider = customProvider?.id === provider ? provider : 'openai'
  ...
  const nextStreamImages = savedDraft?.streamImages ?? (profile.provider === 'openai'
    ...
    : getDefaultStreamImages(targetProvider, nextApiMode))
...
function isLegacyFalProfile(input: unknown): boolean {
  return isRecord(input) && input.provider === LEGACY_FAL_PROVIDER_ID
}
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  if (isLegacyFalProfile(input)) return createDefaultOpenAIProfile()
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const hasProfileList = rawProfiles.length > 0 || record.provider === LEGACY_FAL_PROVIDER_ID
  const survivingRawProfiles = rawProfiles.filter((profile) => !isLegacyFalProfile(profile))
  const profiles = survivingRawProfiles.length
    ? survivingRawProfiles.map((profile) => normalizeApiProfile(profile, undefined, customProviderIds))
    : hasProfileList
...
export function importCustomProviderSettingsFromJson(
  ...
  if (Array.isArray(record.customProviders)) {
    ...
    const profiles = Array.isArray(record.profiles)
      ? record.profiles
        .filter((item) => !isLegacyFalProfile(item))
        .map((item) => {
